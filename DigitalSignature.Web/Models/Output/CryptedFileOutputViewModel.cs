...
namespace DigitalSignature.Web.Models.Output
{
    public class CryptedFileOutputViewModel : OutputViewModel
    {
        ...
        public override void Write()
        {
            ...
            using (StreamWriter cypherTextFile =
                new StreamWriter(Environment.CurrentDirectory + Constants.File.Path.CYPHERTEXT + FileName))
            {
                ...
                for (int i = 0; i < GetNumberOfLines(Data.Length); i++)
                {
                    if ((Data.Length - i * Constants.ROW__CHARACTER_COUNT) < Constants.ROW__CHARACTER_COUNT)
                        cypherTextFile.WriteLine(Constants.TAB + Data.Substring(i * Constants.ROW__CHARACTER_COUNT,
                                                     (Data.Length - i * Constants.ROW__CHARACTER_COUNT)));
                    else
                        cypherTextFile.WriteLine(Constants.TAB + Data.Substring(i * Constants.ROW__CHARACTER_COUNT,
        ...
        {
            StreamReader CypherTextFile =
                new StreamReader(Environment.CurrentDirectory + Constants.File.Path.CYPHERTEXT + FileName);
            ...
            //read data
            while (((CurrentLine = CypherTextFile.ReadLine()) != Constants.END) && (CurrentLine != ""))
                CypherText += CurrentLine.Substring(4);
